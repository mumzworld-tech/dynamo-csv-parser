// This script will parse the data from source | FOR YEAR 2025 ONLY
const fs = require("fs");
...
const outputExcel = path.join("output", `reviews_2025_${timestamp}.xlsx`);
...
(async function processFiles() {
  try {
    for (const file of files) {
      ...
      for await (const line of rl) {
        ...
        try {
          ...
          // Check if created_at is from year 2025
          if (flat.created_at && flat.created_at.slice(0, 4) === targetYear) {
            // Add created_at_converted column
            flat.created_at_converted = flat.created_at.slice(0, 10);
            
            // Add month column
            flat.month = new Date(flat.created_at).toLocaleString('default', { month: 'long' });
      ...
      console.log(`✅ Finished file: ${file} | Total lines: ${lineCount} | Matched 2025: ${matchedCount}`);
    }

    if (filteredData.length === 0) {
      console.log("⚠️ No reviews found for year 2025!");
      return;
    }
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Reviews2025");
